        ...
        .then(data => {
            if (data){
                ...
                for (let i = 0; i < data.length; i++) {
                    let command = data[i];
                    let commandOption = {};
                    for (let j = 0; j < command['commandOption'].length; j++) {
    ...
    return (
        ...
        >
            ...
            <ModalContent bg={'teal.50'}>
                ...
                <ModalBody>
                    {controllerCommands.length > 0 && 
                        controllerCommands.map((command, index) => {
                            return (
                                <Center key={'command_'+index} w={'100%'} mb={2}>
                                <Box w={'90%'} borderColor={'teal.500'} borderWidth={1} p={2} borderRadius={5}>
                                    <Flex w={'100%'} >
                                        <Center w={'65%'}>

                                            <Text w={'100%'}> {
                                                // value of the command['commandOption'] where key is 'command' 
                                                command['commandOption'].find(option => option['key'] === 'command')['value']
                                    ...
                                    {command['commandOption'] && command['commandOption'].length > 0 && 
                                        command['commandOption'].map((option, optIndex) => {
                                            return (
                                                <React.Fragment  key={'commandOption'+index+'_'+optIndex}>
                                                    {!option.static  && option['key'] !== 'target' &&
                                                    <Box w={'100%'} mb={2} p={2} borderRadius={'10px'} bg={'teal.100'}>
                                                        <Flex>
                                                            <Center  w = {'50%'} minW={'50%'}>
                                                                <Text w = {'100%'}>
                                                                    {option['key']} : 
                                                            ...
                                                            <Input 
                                                                ...
                                                                onChange={(event) => {
                                                                    let temp = [...commands];
                                                                    temp[index][option['key']] = event.target.value;
                                                                    setCommands(temp);
                                                                }}
                                                            />
                                                        </Flex>
                ...
                <ModalFooter>
                    {props.additionalButton && props.additionalButtonFunction && props.additionalButtonText &&
                        <Button 
                            key={initialRef} 
                            colorScheme="teal" 
                            mr={3} 
                            onClick={()=>{props.additionalButtonFunction();}}>
