...
void handleGET() {
  ...
  int status = 0;
  if (webServer.args() > 0) {
    if(webServer.argName(0).equals("ssid")){
        status++;
        ssid = webServer.arg(0);
    }
    if(webServer.argName(1).equals("ssidIndex")){
        status++;
        ssidIndex = webServer.arg(1).toInt();
    }
...
void tryToSetupViaWebserver(){
  ...
  while(setupted == false){
    ...
    String sResponse,sHeader,sResponse1="",sResponse2="",sResponse3="";
    ...
    else
    {
      ...
      //generieren des unteren Website-Teils
      sResponse3 += String(R"(
        </span>
        <!--Password-->
      ...
      sHeader += "Content-Length: ";
      sHeader += sResponse1.length() + sResponse2.length() + sResponse3.length();
      sHeader += "\r\n";
      sHeader += "Content-Type: text/html\r\n";
      sHeader += "Connection: close\r\n";
      sHeader += "\r\n";
